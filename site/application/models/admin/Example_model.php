...
class Example_model extends CI_Model{
		...
		public function get_all_simple_users()
		{
			...
			$query = $this->db->get($this->config->item('CONFIG_DB_TBL_USER'));
		...
		public function get_all_users_for_pagination($limit, $offset){
			...
			if(count($wh)>0) {
				...
				$query = $this->db->get_where($this->config->item('CONFIG_DB_TBL_USER'), $WHERE);
			}
			else {
				$query = $this->db->get($this->config->item('CONFIG_DB_TBL_USER'));
			}
		...
		{
			return $this->db->get_where($this->config->item('CONFIG_DB_TBL_USER'), array(
				'id' => $id))->row_array();
		}
		...
		{
			$query = $this->db->get($this->config->item('CONFIG_DB_TBL_USER_GROUP'));
			return $result = $query->result_array();
		}
		...
		{
			$this->db->insert($this->config->item('CONFIG_DB_TBL_UPLOADED_FILE'), $data);
			return true;
		}
		...
		public function get_uploaded_files()
		{
			...
			$query = $this->db->get($this->config->item('CONFIG_DB_TBL_UPLOADED_FILE'));
			return $result = $query->result_array();
		}
		...
		{
			$this->db->where('id', $id);
			$file = $this->db->get($this->config->item('CONFIG_DB_TBL_UPLOADED_FILE'))->row_array()['name'];

			unlink($file);

			$this->db->where('id', $id);
			$this->db->delete($this->config->item('CONFIG_DB_TBL_UPLOADED_FILE'));
			return true;
		}
	...
	//  Charts
		...
		{
			$this->db->select('count(*) as users, ' . $this->config->item('CONFIG_DB_TBL_ADMIN_ROLE') . '.admin_role_title');
			...
			$this->db->group_by($this->config->item('CONFIG_DB_TBL_ADMIN') . '.admin_role_id');
			return $this->db->get($this->config->item('CONFIG_DB_TBL_ADMIN'))->result_array();
		}
		...
		{
			$query = $this->db->get_where($this->config->item('CONFIG_DB_TBL_COUNTRY'), array('status', 1));
			return $query->result_array();
		}
