...
class Sqltoci
{
	 ...
	 * @param type $params
	 */
	function init_config($params = array())
	{ //apply config
		if (count($params) > 0) {
	...
	function generate($tables = null)
	{
		...
		if ($this->write_file) {
			...
			if (!$this->file_per_table) {
				$file_path = $this->path . DIRECTORY_SEPARATOR . $this->file_name . '.sql';
		...
		if ($this->write_file) {
			...
			$files = array_diff(scandir($path . DIRECTORY_SEPARATOR, 1), array('.', '..'));

			$seq = 1;
			if (count($files) == 0) {
				$file_path = $path . DIRECTORY_SEPARATOR . str_pad($seq, 3, '0', STR_PAD_LEFT) . '_Create_base.php';
			} else {
				$seq = (int)preg_replace('/[^0-9]/', '', $files[0]);
				$seq++;
				$file_path = $path . DIRECTORY_SEPARATOR . str_pad($seq, 3, '0', STR_PAD_LEFT) . '_Create_base.php';
			}
