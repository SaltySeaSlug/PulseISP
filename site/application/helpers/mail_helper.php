...
// DKIM Signing
	...
	$controller->email->from($settings['email_from'], strip_tags($settings['application_name']));

	$controller->email->to($to);

	$controller->email->subject(strip_tags(trim($subject)));

	$controller->email->message(strip_tags(trim($body)));
	...
	if ($controller->email->send()) {
		//echo '<hr /><pre>'. $controller->email->print_debugger() .'</pre>';
		return "success";
	} else {
