		...
		<div class="card">
			<div class="card-header">
				...
				<div class="d-inline-block float-right">
					<a href="<?= base_url('admin/admin_roles/add'); ?>" class="btn btn-success"><i class="fad fa-plus mr-2"></i><?= trans('add_new_role') ?>
					</a>
				</div>
			</div>

			<div class="card-body p-0">
				<table id="example2" class="table table-hover table-striped no-footer table-sm text-md nowrap dataTable"
					   style="width: 100%">
					<thead>
					<tr>
							...
							<td>
								<div class="btn-group float-right">
									<a href="<?php echo site_url("admin/admin_roles/access/" . $record['admin_role_id']); ?>"
									   class="btn btn-sm btn-info" title="Permissions"><i class="fad fa-sliders-h"></i></a>

									<?php if (!in_array($record['admin_role_id'], array(1))): ?>
										<a href="<?php echo site_url("admin/admin_roles/edit/" . $record['admin_role_id']); ?>"
										   class="btn btn-sm btn-warning" title="Edit"><i class="fad fa-edit"></i></a>
										<a href="<?php echo site_url("admin/admin_roles/delete/" . $record['admin_role_id']); ?>"
										   onclick="return confirm('are you sure to delete?')"
										   class="btn btn-sm btn-danger" title="Delete"><i class="fad fa-trash-alt"></i></a>
									<?php endif; ?>
								</div>
							</td>
						</tr>
